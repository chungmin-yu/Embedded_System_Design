...
int main(int argc, const char *argv[])
{
    ...
    if (strcmp(argv[1], "NYCU_logo.bmp") == 0) {
        image= cv::imread("NYCU_logo.bmp", cv::IMREAD_COLOR);
    }else if (strcmp(argv[1], "advance.png") == 0) {
    ...
    if (strcmp(argv[1], "NYCU_logo.bmp") == 0) {
       cv::cvtColor(image, image_BGR565, cv::COLOR_BGR2BGR565);
        ...
        cv::cvtColor(image, image_BGR565, cv::COLOR_BGRA2BGR565);
    }else {
        cv::cvtColor(image, image_BGR565, cv::COLOR_BGR2BGR565);
    }
    ...
    for (int y = 0; y < image_size.height ; y++) {
        ...
        ofs.write(reinterpret_cast<char*>(image_BGR565.ptr(y)), image_size.width*2);
    }    
